...
import { handleGenerateXHashtags } from "@/app/actions";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { useToast } from "@/hooks/use-toast";
import { Clipboard, Twitter, Sparkles, Check } from "lucide-react";
...
export default function XHashtagGeneratorForm() {
  const [state, formAction] = useActionState(handleGenerateXHashtags, initialState);
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="font-headline text-xl flex items-center gap-2"><Twitter />Hashtag Generator</CardTitle>
        <CardDescription>Generate viral hashtags to boost your reach on X.</CardDescription>
      </CardHeader>
      <CardContent>
        <form ref={formRef} action={formAction} className="space-y-6">
          <div className="space-y-2">
            ...
            <Input
              id="topic"
              name="topic"
              placeholder="e.g., AI Startups, Crypto News"
              required
            />
          </div>
