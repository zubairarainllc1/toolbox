...
import { generateFacebookCaptions } from '@/ai/flows/facebook-caption-generator';
...
export default function FacebookCaptionGeneratorForm() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    setResult(null);
    try {
      const response = await generateFacebookCaptions(values);
      setResult(response);
    } catch (e) {
      toast({
        variant: 'destructive',
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><Facebook /> Facebook Caption Generator</CardTitle>
        <CardDescription>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="e.g., Summer sale, New product launch" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
